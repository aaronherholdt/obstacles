...
// Handler for community courses data
window.onCommunityCoursesReceived = function(courses) {
    try {
        localStorage.setItem('communityCourses', JSON.stringify(courses));
        console.log('Updated localStorage with community courses from server:', courses.length, 'courses');
        
        // If the current course is a community course, update the selectedCourse
        if (selectedCourse && selectedCourse.id >= 1000) {
            const updatedCourse = courses.find(course => course.id == selectedCourse.id);
            if (updatedCourse) {
                selectedCourse = updatedCourse;
                console.log('Updated selectedCourse with latest data:', selectedCourse);
            }
        }
    } catch (error) {
        console.error('Error updating localStorage with community courses:', error);
    }
};
...
function showCompletionDialog() {
    ...
    if (courseId && !isNaN(parseInt(courseId)) && parseInt(courseId) >= 1000) {
        ...
        likeButton.innerHTML = `
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="18" height="18" style="margin-right: 5px;">
        ...
        // Track if the user has already liked
        let hasLiked = false;
...
function updateCourseStats(courseId, incrementPlays = true, incrementLikes = false) {
    ...
    if (!courseId || isNaN(parseInt(courseId)) || parseInt(courseId) < 1000) {
        console.warn(`Course ID ${courseId} is not a community course or is invalid`);
    ...
    try {
        ...
        if (courseIndex === -1) {
            console.warn(`Course with ID ${courseId} not found in localStorage`);
        ...
        localStorage.setItem('communityCourses', JSON.stringify(communityCourses));
        console.log(`Updated localStorage: Course ${courseId} - Plays: ${communityCourses[courseIndex].plays}, Likes: ${communityCourses[courseIndex].likes}`);
        ...
        if (window.socket && window.socket.readyState === WebSocket.OPEN) {
            if (incrementPlays) {
                ...
                console.log(`Sent WebSocket update: Course ${courseId} - Plays: ${communityCourses[courseIndex].plays}`);
            }
            if (incrementLikes) {
                window.socket.send(JSON.stringify({
                    type: 'updateCourse',
                ...
                console.log(`Sent WebSocket update: Course ${courseId} - Likes: ${communityCourses[courseIndex].likes}`);
            }
        } else {
            console.warn('WebSocket not available, stats may not sync with server');
            // Optionally queue updates for later (see Step 4)
        }
        
        return communityCourses[courseIndex];
    } catch (error) {
        console.error("Error updating course stats:", error);
        alert('Failed to update course stats. Please try again or check your browser settings.');
        return null;
    }
}
// Import libraries
