...
// Function to show distance error modal
function showDistanceErrorModal(message) {
    const errorModal = document.getElementById('distance-error-modal');
    const errorMessage = document.getElementById('distance-error-message');
...
if (publishButton && publishDialog && publishForm && cancelPublishBtn) {
    ...
    publishForm.addEventListener('submit', (event) => {
        ...
        const validation = validateMarkerDistance();
        if (!validation.valid) {
            // Close the publish dialog
            publishDialog.classList.remove('active');
            
            // Show the distance error modal
            showDistanceErrorModal(validation.message);
            return;
        }
        ...
        // Use the publishCourse utility function to publish the course
        if (typeof publishCourse === 'function') {
            ...
            // Fallback to direct WebSocket if the utility function isn't available
...
function isUIElement(element) {
    ...
    while (currentElement) {
        ...
        if (currentElement.id === 'ui-controls' || 
            ...
            currentElement.id === 'publish-dialog' ||  // Add publish dialog
            currentElement.classList.contains('modal-content') ||  // Add modal content
            ...
            currentElement.classList.contains('button-group') ||  // Add button group
            currentElement.classList.contains('cancel-btn') ||    // Add cancel button
            currentElement.classList.contains('submit-btn')) {    // Add submit button
            return true;
        }
...
function togglePlayMode() {
    ...
    if (startInPlayMode && isPlayMode) {
        backButton.innerHTML = `
            ...
            ${isCommunitySource ? 'Back to Community Courses' : 'Back to Obstacle Selector'}
        `;
    } else {
        backButton.innerHTML = `
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="24" height="24">
...
function showCompletionDialog() {
    ...
    // Increment plays count for this course if it's a community course
    const updatedCourse = updateCourseStats(courseId, true, false);
    ...
    // Create like button section if this is a community course
    let likeSection;
    if (courseId && !isNaN(parseInt(courseId)) && parseInt(courseId) >= 1000) {
        likeSection = document.createElement('div');
        likeSection.style.marginBottom = '25px';
        likeSection.style.padding = '15px';
        likeSection.style.backgroundColor = 'rgba(0, 0, 0, 0.3)';
        likeSection.style.borderRadius = '8px';
        
        const likeTitle = document.createElement('h3');
        likeTitle.textContent = 'Did you enjoy this course?';
        likeTitle.style.margin = '0 0 15px 0';
        likeTitle.style.color = '#FF4081';
        
        const likeButton = document.createElement('button');
        likeButton.id = 'likeButton';
        
        // Check if user has already liked this course
        const userLiked = window.hasUserLikedCourse ? window.hasUserLikedCourse(courseId) : false;
        
        likeButton.innerHTML = userLiked ? `
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" style="margin-right: 5px; fill: white;">
                <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
            </svg> Liked!
        ` : `
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" style="margin-right: 5px; fill: none; stroke: white; stroke-width: 2;">
                <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
            </svg> Like This Course
        `;
        
        likeButton.style.backgroundColor = userLiked ? '#4CAF50' : '#FF4081';
        likeButton.style.color = 'white';
        likeButton.style.border = 'none';
        likeButton.style.borderRadius = '5px';
        likeButton.style.padding = '10px 20px';
        likeButton.style.cursor = userLiked ? 'default' : 'pointer';
        ...
        likeButton.style.margin = '0 auto';
        ...
        likeButton.addEventListener('click', () => {
            if (!userLiked) {
                ...
                console.log('Updating course stats for course ID:', courseId);
                const updated = updateCourseStats(courseId, false, true);
                console.log('Course stats updated:', updated);
                
                // Mark course as liked in localStorage
                if (window.markCourseAsLiked) {
                    console.log('Marking course as liked in localStorage');
                    window.markCourseAsLiked(courseId);
                    // Set userLiked to true to prevent multiple likes
                    userLiked = true;
                } else {
                    console.warn('markCourseAsLiked function not available');
                }
                
                // Update button state regardless of whether we get updated data back
                likeButton.style.backgroundColor = '#4CAF50';
                likeButton.style.cursor = 'default';
                likeButton.innerHTML = `
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" style="margin-right: 5px; fill: white;">
                        <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                    </svg> Liked!${updated ? ` (${updated.likes})` : ''}
                `;
                console.log('Like button UI updated');
                likeButton.style.transform = 'scale(1.05)';
                setTimeout(() => {
                ...
                // Update the stats display if we have updated course data
                if (updated && likeStats) {
                    likeStats.textContent = `${updated.likes || 1} people liked this course`;
                    console.log('Like stats updated:', updated.likes);
                }
            }
        });
        
        // Add like stats if available from updated course
        if (updatedCourse) {
            const likeStats = document.createElement('p');
            likeStats.textContent = `${updatedCourse.likes || 0} people liked this course`;
            likeStats.style.marginTop = '10px';
            likeStats.style.fontSize = '0.9rem';
            likeStats.style.color = '#aaa';
            
            // Show plays stats
            const playsStats = document.createElement('p');
            playsStats.textContent = `${updatedCourse.plays || 0} total plays`;
            playsStats.style.fontSize = '0.9rem';
            playsStats.style.color = '#aaa';
            
            likeSection.appendChild(likeTitle);
            likeSection.appendChild(likeButton);
            likeSection.appendChild(likeStats);
            likeSection.appendChild(playsStats);
        } else {
            likeSection.appendChild(likeTitle);
            likeSection.appendChild(likeButton);
        }
    ...
    returnButton.addEventListener('click', () => {
        ...
        // Check if the course is from community section (ID >= 1000 indicates community course)
        const isCommunitySource = selectedCourse && selectedCourse.id && !isNaN(parseInt(selectedCourse.id)) && parseInt(selectedCourse.id) >= 1000;
        
        if (isCommunitySource) {
            // If community course, return to community courses page
            window.location.href = 'index.html?showCommunity=true';
        } else {
            // If regular course, return to course selection
    ...
    // Add like section for community courses
    if (likeSection) {
        dialog.appendChild(likeSection);
    }
    // Only add leaderboard section if in play mode
    if (isPlayMode && leaderboardSection) {
...
// Check if the course is from community courses (ID >= 1000 indicates community course)
const courseId = urlParams.get('course');
const isCommunitySource = courseId && !isNaN(parseInt(courseId)) && parseInt(courseId) >= 1000;

backButton.innerHTML = `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="24" height="24">
        <path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/>
    </svg>
    ${startInPlayMode ? (isCommunitySource ? 'Back to Community Courses' : 'Back to Obstacle Selector') : 'Back to Menu'}
`;
...
backButton.addEventListener('click', function() {
    // In play mode with a community course, go to community courses page
    if (startInPlayMode && isPlayMode && isCommunitySource) {
        window.location.href = 'index.html?showCommunity=true';
    }
    // In play mode with a predefined course, go to course selection
    else if (startInPlayMode && isPlayMode) {
        window.location.href = 'index.html?showCourses=true';
    } else {
...
function createTimerDisplay() {
    ...
    // Add start button, restart button, and timer to the container
...
// Function to update course stats (plays and likes)
function updateCourseStats(courseId, incrementPlays = true, incrementLikes = false) {
    // Only update stats for community courses (ID >= 1000)
    if (!courseId || isNaN(parseInt(courseId)) || parseInt(courseId) < 1000) {
        return null; // Not a community course or invalid ID
    }
    
    // Check if we can use the utility function
    if (typeof window.updateCourseStats === 'function') {
        if (incrementPlays) {
            window.updateCourseStats(courseId, 'plays', null);
        }
        if (incrementLikes) {
            window.updateCourseStats(courseId, 'likes', null);
        }
        
        // Fetch the updated course from localStorage for consistent return
        try {
    ...
    // Update stats on the server if WebSocket is available
    if (window.socket && window.socket.readyState === WebSocket.OPEN) {
        if (incrementPlays) {
            window.socket.send(JSON.stringify({
                type: 'updateCourse',
                courseId: courseId,
                field: 'plays',
                value: null  // Server will increment the current value
            }));
        }
        ...
        try {
            const communityCourses = JSON.parse(localStorage.getItem('communityCourses') || '[]');
            const course = communityCourses.find(c => String(c.id) === String(courseId));
            return course || { id: courseId, likes: 1 }; // Return at least an object with likes
        } catch (e) {
            return { id: courseId, likes: 1 }; // Return a minimal course object
        }
    }
    ...
    try {
        ...
        const courseIndex = communityCourses.findIndex(course => String(course.id) === String(courseId));
        if (courseIndex === -1) return null; // Course not found
...
// Function to calculate distance between start and finish markers
function calculateMarkerDistance() {
    if (!startMarker || !finishMarker) {
        return -1; // Invalid distance if markers don't exist
    }
    
    // Calculate 3D distance between markers
    const dx = finishMarker.position.x - startMarker.position.x;
    const dy = finishMarker.position.y - startMarker.position.y;
    const dz = finishMarker.position.z - startMarker.position.z;
    
    return Math.sqrt(dx*dx + dy*dy + dz*dz);
}

// Function to validate marker distance
function validateMarkerDistance(minDistance = 10) {
    const distance = calculateMarkerDistance();
    
    if (distance < 0) {
        return {
            valid: false,
            message: 'Both start and finish markers must be placed'
        };
    }
    ...
    if (distance < minDistance) {
        return {
            valid: false,
            message: `Start and finish markers must be at least ${minDistance} units apart (current: ${distance.toFixed(2)})`
        };
    }
